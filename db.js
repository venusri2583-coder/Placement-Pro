...
// 🔄 1. పాత బటన్ లింక్ ని కొత్త ఎగ్జామ్ కి మళ్ళించడం
app.get('/mock-test', (req, res) => {
    res.redirect('/grand-test-intro');
});

// 🏆 2. గ్రాండ్ ఎగ్జామ్ సబ్మిట్ చేసినప్పుడు రిజల్ట్ లెక్కించడం
app.post('/submit-grand-exam', requireLogin, async (req, res) => {
    try {
        ...
        // 60 ప్రశ్నలకి ఎన్ని కరెక్ట్ అయ్యాయో చెక్ చేస్తున్నాం
        questions.forEach((q, i) => {
        ...
        // డేటాబేస్ లో సేవ్ చేయడం (Dashboard లో గ్రాఫ్స్ కోసం)
        await db.execute(
            'INSERT INTO mock_results (user_id, score, total, topic) VALUES (?, ?, ?, ?)', 
            [req.session.user.id, score, questions.length, "MNC Grand Test"]
        );

        // పాత result.ejs పేజీనే వాడుకుంటున్నాం (డిజైన్ సేమ్ ఉంటుంది)
        res.render('result', { 
            score: score, 
            total: questions.length, 
            reviewData: reviewData, 
            topic: "MNC Grand Test",
            user: req.session.user 
        });
